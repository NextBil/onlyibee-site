...
-- room_state: the member's LIVING-ROOM evolution (plant growth, jellyfish stage,
-- transit miles…) — persisted per account so it follows them across devices.
alter table public.profiles add column if not exists room_state jsonb not null default '{}'::jsonb;
...
returns jsonb language sql security definer set search_path = public as $$
  select coalesce(
    (select jsonb_build_object('username',p.username,'room',p.room,'room_state',p.room_state,'since',p.created_at)
       from public.profiles p where p.user_id = auth.uid()),
    'null'::jsonb);
$$;

-- ============ save_room_state: persist the living-room evolution (plants/pet/miles) ============
create or replace function public.save_room_state(s jsonb)
returns jsonb language plpgsql security definer set search_path = public as $$
declare uid uuid := auth.uid();
begin
  if uid is null then return jsonb_build_object('status','need_login'); end if;
  if length(s::text) > 40000 then return jsonb_build_object('status','too_big'); end if;
  update public.profiles set room_state = s, updated_at = now() where user_id = uid;
  if not found then return jsonb_build_object('status','no_profile'); end if;
  return jsonb_build_object('status','ok');
end; $$;
grant execute on function public.save_room_state(jsonb) to authenticated;
